...
=======
        console.log("------------ DEPOSIT FROM JS -----------------");
        await weth.connect(account1).deposit({ value: "2000000000000000000" });
>>>>>>> 2d212b3d0db65afdc11152463bf5eb2cb35f5b2c

        const Router = await ethers.getContractFactory("Router");
        router = await Router.deploy(pairFactory.address, weth.address);
        router.deployed();
        await token0.addMinter(owner.address, true);
        await token0.mint(account1.address, TokenValue);
        await token1.addMinter(owner.address, true);
        await token1.mint(account1.address, TokenValue);

        await token0.connect(account1).approve(router.address, TokenValue);
        await token1.connect(account1).approve(router.address, TokenValue);

        const token0Value = TokenValue;
        const token1Value = TokenValue / 2;
        const addLiquidityTrx = await router
            .connect(account1)
            .addLiquidity(
                token0.address,
                token1.address,
        ...
        ETHPairAddress = await pairFactory.getPairAddress(
            token0.address,
            weth.address
        );
